...
# 	- API POST XML - Used to post XML to Blue Yonder Integrator
#
# Assumptions:
# None
#
...
#############################################################
# Description: Used to post XML to Blue Yonder Integrator
# MSQL Files:
#	None
# Inputs:
# 	Required:
...
# 	Optional:
# 		request_xml - XML body to post
# 		xml_file - XML file to post
# Outputs:
# 	response_json - The JSON HTTP response
#############################################################

Given I "validate input arguments"
	When I assign "api_endpoint" to variable "required_args"
	Then I execute scenario "API Validate Required Arguments"
	If I verify variable "request_xml" is assigned
	And I verify text $request_xml is not equal to ""
		Then I assign "" to variable "xml_file"
	ElsIf I verify variable "xml_file" is assigned
	And I verify text $xml_file is not equal to ""
		Then I assign "" to variable "request_xml"
	Else I fail step with error message "Must provide request_xml or xml_file"
	EndIf
...
When I "execute HTTP request"
	When I assign "http_post_xml.groovy" to variable "groovy_file"
	Then I execute scenario "Perform Groovy Execution"

Then I "validate the response"
	Given I verify number $response_code is greater than or equal to 200
	And I verify number $response_code is less than 300
	And I echo $response_code $response_json

And I "attempt to parse download sequence from JSON response"
	If I assign value from json $response_json with path "/downloadSequence" to variable "new_download_sequence"
	EndIf
